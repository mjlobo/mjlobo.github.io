...
bootstrapCI <- function(statistic, datapoints, interval) {
  if (missing(interval)) {
    interval = 'bca'
  }
  ...
  if (interval == 'perc') {
    lowerBound <- ci$perc[4]
    upperBound <- ci$perc[5]
  }
  return(c(pointEstimate, lowerBound, upperBound))
}
bootstrapCI_corr <- function(statistic, datapoints, comparisons, interval) {
  if (missing(interval)) {
    interval = 'bca'
  }
  # Compute the point estimate
  ...
  # Compute interval
  #ci <- boot.ci(b, type = intervalMethod)
  ci <- boot.ci(b, type = c("perc", "bca"))
  level <- 1 - 0.05/comparisons
  ci_corr <- boot.ci(b, type =c("perc", "bca"), conf = level)
  # Return the point estimate and CI bounds
  # You can print the ci object for more info and debug
  print(ci)
  ...
  if (interval == 'perc') {
    lowerBound <- ci$perc[4]
    upperBound <- ci$perc[5]
    lowerBound_corr <- ci_corr$perc[4]
    upperBound_corr <- ci_corr$perc[5]
  }
...
bootstrapMeanCI <- function(datapoints, intervalMethod) {
  return(bootstrapCI(samplemean, datapoints, intervalMethod))
}
bootstrapMeanCI_corr <- function(datapoints, comparisons, intervalMethod) {
  return(bootstrapCI_corr(samplemean, datapoints, comparisons, intervalMethod))
}
...
# 2015--201 Wesley Willett, Petra Isenberg and Lonni Besancon, Anastasia Bezerianos

library(ggplot2)
library(reshape2)


barChart <- function(resultTable, techniques, nbTechs = -1, ymin, ymax, mycolor= "black", xAxisLabel = "I am the X axis", yAxisLabel = "I am the Y Label", mysize = 1.1){
  #tr <- t(resultTable)
  if(nbTechs <= 0){
  ...
  g <- ggplot(tr, aes(x=technique, y=mean_time)) + 
    ...
    coord_flip() +
    theme(panel.background = element_rect(fill = 'white', colour = 'white'),axis.title=element_text(size = rel(1.2), colour = "black"),axis.text=element_text(size = rel(1.2), colour = "black"),panel.grid.major = element_line(colour = "gainsboro"),
          panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank(), text = element_text(size=16))+
    geom_point(size=2*mysize, colour=mycolor)         # dots
  print(g)
}


barChart_corr <- function(resultTable, techniques, nbTechs = -1, ymin, ymax, mycolor = "black", xAxisLabel = "I am the X axis", yAxisLabel = "I am the Y Label"){
  ...
  g <- ggplot(tr, aes(x=technique, y=mean_time)) + 
    ...
    scale_x_discrete(name="",breaks,techniques)+
    coord_flip() +
    theme(panel.background = element_rect(fill = 'white', colour = 'white'),axis.title=element_text(size = rel(1.2), colour = "black"),axis.text=element_text(size = rel(1.2), colour = "black"),panel.grid.major = element_line(colour = "gainsboro"),panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank())+
    geom_point(size=2, colour=mycolor)         # dots
